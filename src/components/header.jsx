...
class ResourceBtn extends React.Component {
  ...
  render() {
    ...
    // Extract base key for icon lookup (e.g., "paper" from "paper (Coming Soon)")
    const baseKey = this.props.title.split(' ')[0].toLowerCase();
    const FaIcon = this.icons[baseKey];
    const iTitle =
      baseKey == 'huggingface' && this.state.isMobile
        ? ' hf '
        : this.props.title;
    return (
      <>
